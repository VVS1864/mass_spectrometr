...
public class Graph_mass extends Graph_canvas {
	...
	void draw_data() {
		...
		for (int mass_first = 0; mass_first < Run.prog.fixed_data_mass_intensity.length; mass_first++) {
			...
			for (int mass_second = mass_first
					+ 1; mass_second < Run.prog.fixed_data_mass_intensity.length; mass_second++) {
		...
		/*
		 * if(x_data.size()>2 && Run.prog.draw_graph_mass == false) { analyser = new
